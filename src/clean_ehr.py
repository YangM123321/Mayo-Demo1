...
def main(in_path="data/labs_raw.csv", out_path="out/labs_clean.parquet"):
    ...
    df["unit"] = (
        df["unit"].astype(str).str.strip().str.lower().map(UNIT_NORMALIZATION).fillna(df["unit"])
    )
    df["loinc"] = df["lab_code"].map(LOCAL_TO_LOINC).fillna(df["lab_code"])

    df = df.sort_values("collected_date").drop_duplicates(
        subset=["patient_id", "encounter_id", "loinc"], keep="last"
    )
